...
def show():
    ...
    for dt, r in regimes.items():
        if r != prev_r:
            fig.add_vrect(x0=start_dt, x1=dt,
                          fillcolor=REGIME_COLORS.get(prev_r, "#888") + "18",
    ...
    with col1:
        ...
        signal_data = [
            ...
            (f"Momentum 3M = {s.get('mom3m', 0)*100:.1f}%", "Positive" if (s.get("mom3m") or 0) > 0 else "Negative"),
            (f"Momentum 6M = {s.get('mom6m', 0)*100:.1f}%", "Positive" if (s.get("mom6m") or 0) > 0 else "Negative"),
    ...
    with col2:
        ...
        fig_t = go.Figure(go.Heatmap(
            ...
            colorscale="Blues", text=np.round(z * 100, 1),
    ...
    if not probs.empty and probs.shape[1] > 1:
        ...
        for col in probs.columns:
            c = REGIME_COLORS.get(col, "#888")
            fig_prob.add_trace(go.Scatter(
                x=probs.index, y=probs[col].values, mode="lines",
                stackgroup="one", name=col,
                line=dict(width=0.5, color=c), fillcolor=c + "88",
            ))
        fig_prob.update_layout(height=200, template="plotly_dark", paper_bgcolor="rgba(0,0,0,0)",
                                plot_bgcolor="rgba(0,0,0,0)", margin=dict(l=0,r=0,t=10,b=0),
